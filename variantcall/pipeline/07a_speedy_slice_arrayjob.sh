...
do
    ...
    echo $outstrain
    printf ">%s\n%s\n" $outstrain $(bcftools query -e 'INFO/AF < 0.1' -s "$outstrain" -f '[%TGT]' ${vcf}) > $TMPOUT/$PREFIX.$TYPE.$outstrain.fas_seq
    perl -i -p -e 'if (/^>/) { s/[\(\)#]/_/g; s/_+/_/g; } else { s/[\*\.]/-/g; }' $TMPOUT/$PREFIX.$TYPE.$outstrain.fas_seq
